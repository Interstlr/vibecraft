// Based on standard Perlin Noise implementations
export class Noise {
  private p: number[] = [];
  ...
  setSeed(seed: number) {
    // Simple LCG to shuffle permutation based on seed
    const lcg = (s: number) => (s * 1664525 + 1013904223) % 4294967296;
    let s = seed;
    
    const pSource = [...this.permutation];
    this.p = new Array(512);
    
    // Shuffle
    for (let i = 0; i < 256; i++) {
        s = lcg(s);
        const r = Math.abs(s) % pSource.length;
        const val = pSource[r];
        pSource.splice(r, 1);
        this.p[i] = val;
        this.p[i + 256] = val;
